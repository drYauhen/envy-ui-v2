...
- `tokens:build:sd`: Build Style Dictionary platform outputs. The themed CSS formatter writes `generated/css/tokens.themed.css`. Resolver raw-alias normalization is always-on for resolver targets and unresolved references fail the build (no fail-soft fallback path).
...
- `resolver:check`: CI-ready resolver integrity check (schema + phase validations + canonical build + themed CSS snapshot regression).
- `resolver:validate:schema`: Validate all `*.resolver.json` files against local JSON schema (`schemas/dtcg-resolver-2025.10.schema.json`).
