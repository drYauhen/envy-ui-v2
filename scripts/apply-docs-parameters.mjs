...
/**
 * Script to apply section parameters to all stories in Docs section (ADR and Architecture)
...
// Find all story files in Docs section
const storyFiles = glob.sync('stories/docs/**/*.stories.tsx', {
  cwd: projectRoot,
  absolute: true
});

console.log(`Found ${storyFiles.length} story files in Docs section`);
...
for (const filePath of storyFiles) {
  try {
    ...
    // Skip if title doesn't start with "Docs/"
    ...
    if (parametersMatch) {
      ...
      // But preserve existing parameters (like layout, interactions, etc.)
