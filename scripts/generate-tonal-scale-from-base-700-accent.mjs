 ...
 * 
 * Approach for Accent color (700 anchor):
 ...
 * 3. Primary usage range: Accent-50 → Accent-700
 * 4. Accent-800-900 as edge-cases only
...
function getAsymmetricLightnessStepsFor700() {
  ...
  // For Accent-700 anchor (medium base ~50%):
...
function generateScaleFrom700(base700, config) {
  ...
  let currentL = base.l; // Start at 700
  const upValues = [];
  ...
  // upValues should now be: [50, 100, 200, 300, 400, 500, 600] in correct order
...
function processAccentFile(filePath) {
  ...
  // Get base 700 (canonical accent color)
  ...
  Object.entries(scale).forEach(([level, oklch]) => {
    ...
    if (level === '700') {
      data.eui.color[colorFamily][level].$description = `Base accent color - canonical accent color (anchor)`;
...
console.log('\n📋 Best Practices:');
console.log('  • Base 700 preserved as canonical accent color (anchor)');
console.log('  • Asymmetric distribution: more light shades (50-600), fewer dark (800-900)');
console.log('  • Primary usage range: Accent-50 → Accent-700');
console.log('  • Accent-800-900 as edge-cases only');
